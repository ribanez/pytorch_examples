...
def parser_data(aa, path_to_pdbs, path_to_csv):
    ...
    for f in files:
        ...
        print(name[:-4])
        if not os.path.exists(path_to_csv + 'pdb-parsed' + name[:-4] + '.csv') and not os.path.exists(path_to_csv + 'pdb-rejected' + name[:-4] + '.csv'):
            try:
                structure = p.get_structure('X', f)
            ...
            if len(df) > 0:
                print(name[:-4] + ' success')
                df.to_csv(path_to_csv + 'pdb-parsed' + name[:-4] + '.csv')
            else:
                df.to_csv(path_to_csv + 'pdb-rejected' + name[:-4] + '.csv')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Get folders to PDBs')
    parser.add_argument("-f", "--folder", help="PDB folders")
    args = parser.parse_args()

    if args.folder is not None:
        path_to_pdb = args.folder
    else:
        raise ValueError("")
    ...
    parser_data(aa, path_to_pdb, path_to_csv)
